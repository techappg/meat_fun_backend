...
class checkSendOTPEmailApi(GenericAPIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if user.exists():
                ...
                return Response('email dose not exists', status.HTTP_400_BAD_REQUEST)
            ...
            if resp_status == "success":
                ...
                data = {
                    "otp": "success",
                    "mobile_no": email,
                    "user_status": User.objects.filter(email=email).exists()
                }
            else:
